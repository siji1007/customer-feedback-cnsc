...
const handleOpenModal = (index: number) => {
  ...
  const fetchDepartments = async () => {
    try {
      const response = await axios.get<{ departments: string[] }>(serverUrl + "academic_department");
  ...
  useEffect(() => {
    const fetchOffices = async () => {
      try {
        ...
        const response = await axios.get<{ departments: Office[] }>(serverUrl + 'service_department');
  ...
  const handleSaveClick = () => {
    ...
    const updatedQuestionnaires = [...questionnaires[selectedOffice.name] || []];
    updatedQuestionnaires[selectedQuestionnaire].questions[editIndex!] = editQuestion;
    setQuestionnaires({ ...questionnaires, [selectedOffice.name]: updatedQuestionnaires });
    setEditIndex(null);
  };
            ...
            <div className="flex flex-col md:flex-row h-full overflow-hidden">
              ...
              <div className="w-full border-b md:border-b-0 md:border-r rounded-lg p-4 ml-2" style={{ height: "40vh", background: "#c3c3c3" }}>
                ...
                <ul className="space-y-2 text-black rounded-lg p-2">
                  {selectedOffice && (
                    <>
                      {questionnaires[selectedOffice.name] && questionnaires[selectedOffice.name].length > 0 ? (
                        <>
                          {questionnaires[selectedOffice.name].map((questionnaire, index) => (
                            <li key={index} className={`flex items-center justify-between p-2 border border-white rounded-lg ${selectedQuestionnaire === index ? "bg-red-800 text-white" : "hover:bg-gray-100"}`} onClick={() => handleOpenModal(index)}>
                            <span>{questionnaire.name}</span>
                            <button className="text-red-600" onClick={(e) => { e.stopPropagation(); handleDeleteQuestionnaire(index); }}>
                              <FaTimes />
                            </button>
                          </li>
                          ))}
                        ...
                        <li className="p-2 text-gray-600">No questionnaires found.</li>
                      )}
                   <li className="p-2">
                    <div className="flex flex-col items-center">
                      <input type="text" value={newQuestionnaireName} onChange={(e) => setNewQuestionnaireName(e.target.value)} placeholder="New Questionnaire Name" className="w-full px-2 py-1 rounded-lg border border-gray-300 focus:outline-none focus:border-red-800"/>
                      <button className="mt-2 bg-red-800 text-white py-1 px-4 rounded-lg" onClick={handleAddQuestionnaire}>
                        Add Questionnaire
                      </button>
                    </div>
                  </li>
              ...
              <Modal
                ...
                >
                  <div className="bg-white p-6 rounded-lg shadow-lg max-w-md sm:max-w-lg mx-auto w-full">
                            ...
                            {editIndex === index ? (
                              <>
                                ...
                                <div className="flex space-x-2 mt-2">
                                  <FaSave className="cursor-pointer w-6 h-6" onClick={handleSaveClick} />
                                  <FaTimes className="cursor-pointer w-6 h-6" onClick={handleCancelClick} />
                                </div>
                              </>
                            ) : (
                              <>
                                <span className="break-words">{question}</span>
                                <FaEdit className="cursor-pointer mt-2 w-6 h-6" onClick={() => handleEditClick(index, question)} />
                              </>
                            )}
                          </li>
                        ))}
                    ...
                    <button className="mt-4 w-full bg-red-800 text-white py-1 rounded-lg" onClick={handleCloseModal}>
                      Close
                    </button>
                  </div>
                </Modal>
