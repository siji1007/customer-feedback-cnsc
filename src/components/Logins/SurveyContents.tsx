...
const SurveyContents: React.FC = () => {
    ...
    const handleDrag = (e: React.MouseEvent<HTMLDivElement>, questionIndex: number) => {
    ...
    const getScaleValue = (questionIndex: number) => {
        ...
        const position = positions[questionIndex] || 0;
        const index = Math.floor(position / scaleWidth);
        const values = ['Very Satisfied', 'Satisfied', 'Neutral', 'Dissatisfied', 'Very Dissatisfied'];
        return values[index];
    };
                    ...
                    >
                        <div className="absolute inset-0 h-10 md:h-12 bg-gradient-to-r from-green-400 via-yellow-400 to-red-400 rounded flex items-center overflow-hidden">
                            <div
                                ...
                                style={{ left: `${positions[questionIndex] || 0}%`, transition: 'background-color 0.2s' }}
                            ></div>
                        </div>
                        <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 text-center">
                            <span className="text-xs md:text-sm lg:text-lg font-semibold whitespace-nowrap">{getScaleValue(questionIndex)}</span>
                        </div>
                    </div>
                </form>
            );
