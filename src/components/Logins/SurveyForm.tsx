      ...
      {content === 'Instruction' ? (
        <>
          ...
          <div className='flex justify-end mt-10'>
            <button
              className='px-4 py-2 bg-red-900 text-white rounded-lg text-sm md:text-base lg:text-lg'
...
<>
    ...
    {['Admission Office', 'Registrar Office', 'Guidance Office', 'Health Service Office', 'Library', 'Canteen (Food Service)', 'Student Publication', 'Scholarship Programs', 'Student Organization Sport and Cultural Services'].map((office, index) => (
      <label key={index} className="flex items-center justify-between">
        ...
        <input
          ...
          name={office} // Use a unique name for each checkbox
          value={office}
          onChange={handleCheckboxChange} // Replace with your handler function
          className="ml-2"
        />
      </label>
    ))}
  ...
  <div className="flex justify-between mt-10"> {/* Changed to justify-between */}
    <button
      className="px-4 py-2 bg-red-900 text-white rounded-lg text-sm md:text-base lg:text-lg mr-4" // Add margin for spacing
